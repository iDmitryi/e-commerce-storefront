...
import { removeProduct, selectProducts, selectTotal } from './favouriteSlice.ts'
import { TrashIcon } from '@heroicons/react/16/solid'

const FavouriteDropdown: FC = () => {
  const dispatch = useAppDispatch()
  const total = useAppSelector(selectTotal)
  const products = useAppSelector(selectProducts)

  return (
    <Popover className="flow-root text-sm lg:ml-8 lg:relative">
      <PopoverButton className="group flex items-center p-2">
        ...
        <span className="sr-only">items in favourites, view favourites</span>
      </PopoverButton>
      <PopoverPanel
        transition
        ...
        <form className="mx-auto max-w-2xl px-4 max-h-[500px] overflow-auto no-scrollbar">
          <ul role="list" className="divide-y divide-gray-200">
            {products.length ? (
              ...
              <li className="py-6">No items in favourites</li>
            )}
          </ul>
        </form>
      </PopoverPanel>
